...
public:
    ...
    void update()
    {
        ...
        if (numPackets > 0) 
        {
            ...
            if (signalQuality == 0.0) 
            {
                ...
                if (TG_GetValueStatus(connectionID, TG_DATA_RAW) != 0.0)	//JH added
                {
                    float raw = TG_GetValue(connectionID, TG_DATA_RAW);
                    ofNotifyEvent(rawChangeEvent, raw);
                }
    ...
    ofEvent<float> rawChangeEvent;		//JH added
    ...
    ofEvent<float> gamma2ChangeEvent;	//JH end
